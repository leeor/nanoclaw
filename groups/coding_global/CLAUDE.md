...
Whenever you observe — by any means (a "PR merged" message from the monitor, a
...
`CLOSED`, your next action MUST be to invoke `mcp__nanoclaw__cleanup_coding_task`.

Do not assume cleanup is "automatic" and wait for it. The orchestrator runs an
orphan sweep as a safety net, but the agent calling cleanup directly is the
primary path. Calling it twice is safe (the second call is a no-op).
