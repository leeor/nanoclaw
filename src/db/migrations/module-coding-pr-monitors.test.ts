...
describe('coding-pr-monitors migration', () => {
  ...
  it('creates coding_pr_monitor_seen with composite PK', () => {
    const cols = getDb().prepare("PRAGMA table_info('coding_pr_monitor_seen')").all() as Array<{
      name: string;
      pk: number;
    }>;
  ...
  it('cascades seen rows when monitor is deleted', () => {
    ...
    db.prepare(`INSERT INTO coding_pr_monitor_seen (monitor_id, comment_id, updated_at) VALUES ('m1', 100, ?)`).run(
      now(),
    );
    expect((db.prepare('SELECT COUNT(*) AS c FROM coding_pr_monitor_seen').get() as { c: number }).c).toBe(1);
    db.prepare('DELETE FROM coding_pr_monitors WHERE id = ?').run('m1');
    expect((db.prepare('SELECT COUNT(*) AS c FROM coding_pr_monitor_seen').get() as { c: number }).c).toBe(0);
  });
  ...
  it('cascades monitor + seen rows when agent group is deleted', () => {
    ...
    expect((db.prepare('SELECT COUNT(*) AS c FROM coding_pr_monitors').get() as { c: number }).c).toBe(0);
    expect((db.prepare('SELECT COUNT(*) AS c FROM coding_pr_monitor_seen').get() as { c: number }).c).toBe(0);
  });
});
